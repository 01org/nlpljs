...
(function (_, ArrayUtils) {
  ...
  var findSequences = function (arr, startIndex, fn, length) {
    ...
    for (var index = startIndex; index < arr.length; index++) {
      ...
      if (fn(item)) {
        ...
        inProgress.push([]);  // append this item to all existing sequences
  ...
  var findColumnSequences = function (row, width) {
    var startColumn = 0;
    return findSequences(row, startColumn, function (item) {
      return !item;
    }, width);
  ...
  var findSpaces = function (grid, width, height) {
    ...
    var rowSequences = findRowSequences(grid, height);  // this contains all possible row+column combinations which could
                                                        // fit width x height
    // this contains all possible row+column combinations which could
    // fit width x height
    var candidates = [];  // for each row sequence, get the column sequences for each of its rows
    ...
    for (var i = 0; i < rowSequences.length; i++) {
      ...
      // sequences of columns common across all of the rows
      var commonColumnSequences = ArrayUtils.intersect(_.values(columnSequencesByRow));  // data structure looks like this:
...
}(typeof _ === 'undefined' ? require('../bower_components/lodash/dist/lodash') : _, typeof ArrayUtils === 'undefined' ? require('./cp-array-utils') : ArrayUtils));
