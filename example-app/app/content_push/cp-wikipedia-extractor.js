...
(function (_, HtmlCleaner) {
  ...
  var httpRegex = /^http:/;  /* return chunks matching regex; regex should have a capturing
  ...
  var getImages = function (html, minImageSide, numImages) {
    var imgs = imageRegex.exec(html);  /* no images */
    /* no images */
    if (!imgs) {
      return [];
    }
    ...
    for (var i = 0; i < imgs.length; i++) {
      img = imgs[i];  /* prevent image from being used unless width and height are set
      ...
      if (height >= minImageSide && width >= minImageSide) {
        ...
        if (srcMatches) {
          src = srcMatches[1];  /* Wikipedia images sometimes have the scheme missing
...
}(typeof _ === 'undefined' ? require('../bower_components/lodash/dist/lodash') : _, typeof HtmlCleaner === 'undefined' ? require('./cp-html-cleaner') : HtmlCleaner));
