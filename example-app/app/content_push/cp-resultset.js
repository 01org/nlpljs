...
(function () {
  ...
  var ResultSet = function (query) {
    ...
    this.query = query;  /* cursor position inside the result set to retrieve from next */
    /* cursor position inside the result set to retrieve from next */
    this.cursor = 0;  /* offset for next search for this query */
    /* offset for next search for this query */
    this.offset = 0;  /* set to true if no more results are available for this query */
    /* set to true if no more results are available for this query */
    this.isExhausted = false;  /* the actual results; each is an object with a src property
  ...
  ResultSet.prototype.fetch = function (numResults) {
    ...
    this.cursor = Math.min(this.cursor + numResults, this.results.length);
    return batch;
