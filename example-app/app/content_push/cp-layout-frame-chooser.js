...
(function () {
  ...
  */
  ...
  FrameChooser.prototype.choose = function (imageItem) {
    // best so far
    var nearestRatio;
    var nearestSize;  // chosen frame so far
                      // chosen frame so far
                      // chosen frame so far
                      // chosen frame so far
    // chosen frame so far
    ...
    var sizeDiff;
    var heightDiff;
    var widthDiff;  // the image must be the same height or taller than the frame
    ...
    for (var i = 0; i < FRAMES.length; i++) {
      ...
      betterCandidate = !chosenFrame || ratioDiff < nearestRatio && heightGreaterOrEqual || ratioDiff === nearestRatio && sizeDiff < nearestSize && heightGreaterOrEqual;
      if (betterCandidate) {
