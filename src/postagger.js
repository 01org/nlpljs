...
(function () {
  var makePostagger = function (tokenizer, modelJSON) {
    var model;
    var stopSymbols = ['.', '?', '!', ';'];
  ...
  if (typeof define !== 'undefined' && define.amd) {
    define(['tokenizer', 'text!models/english.json'], function (tokenizer, englishModel) {
      return makePostagger(tokenizer, englishModel);
    });
  } else {
    var fs = require('fs');
    ...
    var modelFilePath = path.join(__dirname, './models/english.json');
    var englishModel = fs.readFileSync(modelFilePath, 'utf8');

    module.exports = makePostagger(tokenizer, englishModel);
  }
})();
